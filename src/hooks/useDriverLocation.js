...
const useDriverLocation = (isOnline = false) => {
    ...
    async (latitude, longitude) => {
      ...
      try {
        ...
        if (supabaseError) {
          ...
          console.log(
            `📍 Location updated: (${latitude.toFixed(4)}, ${longitude.toFixed(
              4
            )})`
          );
        }
  ...
  const simulateRoute = useCallback(
    (points, duration = 60000) => {
      ...
      // Use shorter interval (2 seconds) for smoother movement
      const INTERVAL_MS = 2000;

      // Calculate how many points to move per interval
      // Limit to max 3 points per interval to avoid jumping too fast
      const totalIntervals = Math.floor(duration / INTERVAL_MS);
      const pointsPerInterval = Math.min(
        3, // Max 3 points per interval for smooth movement
        Math.max(1, Math.floor(points.length / totalIntervals))
      );
      ...
      console.log(`🚗 Starting simulation:`, {
        ...
        pointsPerInterval,
        estimatedSteps: Math.ceil(points.length / pointsPerInterval),
        firstPoint: points[0],
