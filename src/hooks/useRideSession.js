...
const useRideSession = (
      ...
      if (response?.data?.data) {
        ...
        if (data.routePolyline) {
          ...
          try {
            ...
            setRoutePoints(
              decoded.map((p) => ({ latitude: p[0], longitude: p[1] }))
            );
          } catch (e) {
        ...
        (payload) => {
          ...
          if (newPriority >= currentPriority) {
            if (updated.status !== rideStatus) {
              console.log(
                `✅ Status update: ${rideStatus} → ${updated.status}`
              );
              setRideStatus(updated.status);
            }
          } else {
            console.warn(
              `⚠️ Ignoring status rollback: ${updated.status} (priority ${newPriority}) < ${rideStatus} (priority ${currentPriority})`
            );
          }
  ...
  useEffect(() => {
    ...
    if (isDriver || !targetDriverId) {
      console.log(
        "⏭️ Skipping driver location subscription:",
        isDriver ? "is driver" : "no targetDriverId"
      );
      return;
    }

    console.log(
      "🔔 Subscribing to driver location for driverId:",
      targetDriverId
    );
    ...
    const fetchDriverLocation = async () => {
      try {
        ...
        if (data) {
          ...
          console.warn(
            "⚠️ No driver location found for driverId:",
            targetDriverId
          );
        }
      } catch (err) {
    ...
    try {
      ...
      if (response?.data?.data) {
        ...
        try {
          ...
          console.warn(
            "⚠️ Failed to update Supabase, but API succeeded:",
            supabaseErr
          );
        }
